...
function buildItems(pool: (string | ImageObj)[], seg: number): GridItem[] {
  ...
  for (let i = 1; i < usedImages.length; i++) {
    ...
    const curr = usedImages[i];
    if (curr && prev && curr.src === prev.src) {
      for (let j = i + 1; j < usedImages.length; j++) {
        const next = usedImages[j];
        if (next && next.src !== curr.src) {
...
export function DomeGallery({
  ...
  maxRadius = Infinity,
  padFactor = 0.25,
  overlayBlurColor = '#050505', // Match EN2H secondary background
  ...
  imageBorderRadius = '1px', // Match EN2H sharp card border radius
          ...
          <div ref={sphereRef} className="sphere">
            {items.map((it, i) => (
              ...
              >
                ...
                >
                  <Image src={it.src} draggable={false} alt={it.alt ?? ''} fill sizes="(max-width: 768px) 45vw, 240px" unoptimized />
                </div>
              </div>
            ))}
          </div>

...
export const DotField = memo(({
  ...
  sparkle = false,
  waveAmplitude = 0,
  gradientFrom = 'rgba(197, 166, 124, 0.35)', // Match EN2H primary #C5A67C
  gradientTo = 'rgba(197, 166, 124, 0.15)',
  glowColor = '#050505',
  ...rest
}: DotFieldProps) => {
  ...
  useEffect(() => {
    ...
    function buildDots(w: number, h: number) {
      ...
      const rad = p.dotRadius ?? 1.5;
      const sp = p.dotSpacing ?? 14;
      const step = rad + sp;
    ...
    function tick() {
      ...
      grad.addColorStop(0, p.gradientFrom ?? 'rgba(197, 166, 124, 0.35)');
      grad.addColorStop(1, p.gradientTo ?? 'rgba(197, 166, 124, 0.15)');
      ctx.fillStyle = grad;
      ...
      for (let i = 0; i < len; i++) {
        ...
        if (distSq < crSq && eng > 0.01) {
          ...
          if (isBulge) {
            ...
            const push = tempT * tempT * (p.bulgeStrength ?? 67) * eng;

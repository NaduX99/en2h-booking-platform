...
export class BookingsService {
  ...
  async create(createBookingDto: CreateBookingDto) {
    await this.servicesService.findActiveEntityById(createBookingDto.serviceId);
  ...
  async findAll(query: BookingQueryDto) {
    ...
    if (query.serviceId) {
      queryBuilder.andWhere('booking.serviceId = :serviceId', {
        serviceId: query.serviceId,
      });
    }

    if (query.customerEmail) {
      queryBuilder.andWhere('booking.customerEmail = :customerEmail', {
        customerEmail: query.customerEmail.trim().toLowerCase(),
      });
    }
    ...
    if (query.search?.trim()) {
      ...
      queryBuilder.andWhere(
        new Brackets((qb) => {
          qb.where('LOWER(booking.customerName) LIKE :search', { search })
            .orWhere('LOWER(booking.customerEmail) LIKE :search', { search })
            .orWhere('LOWER(booking.customerPhone) LIKE :search', { search })
            .orWhere('LOWER(service.title) LIKE :search', { search });
        }),
      );
    }
  ...
  async updateStatus(id: string, newStatus: BookingStatus) {
    ...
    if (booking.status === BookingStatus.COMPLETED) {
      throw new BadRequestException('Completed bookings cannot be updated');
    }
  ...
  private validateFutureBooking(bookingDate: string, bookingTime: string) {
  ...
  private isUniqueViolation(error: QueryFailedError) {
